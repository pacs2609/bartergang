            ...
            <div class="col-md-5 product-detail-content d-flex flex-column gap-2">
                <div class="row">
                    <div class="col-md-6 border p-4 bg-border">
                        ...
                        <span class="fs-6 fw-600" v-if="authStore.isAuthenticated">{{ formatNumber(authStore.user?.bbcoin) }}</span>
                        <span class="fs-6 fw-600" v-if="!authStore.isAuthenticated">0</span>
                    </div>
                    <div class="col-md-6 border p-4 tmc-border">
                        <img src="/newimg/tmc.png" alt="">
                        <span class="fs-6 fw-600" v-if="authStore.isAuthenticated">{{ formatNumber(authStore.user?.tmccoin) }}</span>
                        <span class="fs-6 fw-600" v-if="!authStore.isAuthenticated">0</span>
                    </div>
                    <div class="col-md-12 d-flex justify-content-between mt-4">
                        <span class="fw-600">{{ formatNumber(productData?.bbprice) }}</span>
                        <span class="fw-600 text-primary">BB</span>
                    </div>
                    <div class="col-md-12 d-flex justify-content-between mt-4">
                        <span class="fw-600">{{ formatNumber(productData?.tmcprice) }}</span>
                        <span class="fw-600 text-primary">TMC</span>
                    </div>
                    <div class="col-md-12 mt-3 d-flex justify-content-between">
                        <span class="fw-600 text-primary">In Stock {{ productData?.quantity }}</span>
                        <span class="fw-600" v-if="productData?.newProduct == 'N'">สินค้ามือสอง <span class="text-primary fw-600">[{{ productData?.productPercent }}%]</span></span>
                        <span class="fw-600" v-if="productData?.newProduct == 'Y'">สินค้าใหม่</span>
                    </div>
                    ...
                    <div class="col-md-12 mt-2 d-flex gap-2 align-items-center">
                        ...
                        <QuantitySelector :max="productData?.quantity" :center="true" v-model="currentQuantity" @change="handleQuantityChange"/>
                    ...
                    <div class="col-md-12 d-flex justify-content-end">
                        <span class="fw-500 fs-6">วันที่ลงประกาศ : {{ productData?.createat }}</span>
                    </div>
                </div>
            </div>
...
const {
    ...
    pending,
    error
} = await useFetch < Product[] > ('/api/products/getNewarrival');
